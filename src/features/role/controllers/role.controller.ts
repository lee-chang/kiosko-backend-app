...
export class RoleController {
  ...
  static async getRolesByFor(req: Request, res: Response, next: NextFunction) {
    const { forType } = req.params
    let page = Number(req.query.page) || 1
    let limit = Number(req.query.limit) || Infinity

    try {
      const role = await RoleSevice.findRoleByFor(forType, page, limit)
      res.status(HttpStatus.OK).json(role)
    } catch (err) {
      next(err)
    }
  ...
  static async updatePermissions(
    ...
    const { id } = req.params
    const { permissions }: { permissions: KeyPermissionsType[] } = req.body
